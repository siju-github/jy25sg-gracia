  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user && user.email) {
        ...
        try {
          const approved = await checkIsAdminApproved(user.email, false);
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-white/70 leading-relaxed px-4">
              {enableGoogleLogin && enablePassIdLogin && enableEmailLogin && (
                "Sign in with your Google account or authenticate using your unique Scripture Pass ID and registered email."
              )}
              {enableGoogleLogin && enablePassIdLogin && !enableEmailLogin && (
                "Sign in with your Google account or authenticate directly using your unique Scripture Pass ID."
              )}
              {enableGoogleLogin && !enablePassIdLogin && enableEmailLogin && (
                "Sign in with your Google account or search for your pass using your registered email address."
              )}
              {enableGoogleLogin && !enablePassIdLogin && !enableEmailLogin && (
                "Sign in with your Google account to access your digital pass and registration details."
              )}
              {!enableGoogleLogin && enablePassIdLogin && enableEmailLogin && (
                "Authenticate using your unique Scripture Pass ID and registered email address."
              )}
              {!enableGoogleLogin && enablePassIdLogin && !enableEmailLogin && (
                "Authenticate directly using your unique Scripture Pass ID."
              )}
              {!enableGoogleLogin && !enablePassIdLogin && enableEmailLogin && (
                "Enter your registered email address to locate and access your digital pass."
              )}
              {!enableGoogleLogin && !enablePassIdLogin && !enableEmailLogin && (
                "Sign in with your Google account to access your digital pass."
              )}
            </p>
          </div>
          ...
          {/* Option A: Google OAuth (Show if enabled OR if all toggles are false as fallback) */}
          {(enableGoogleLogin || (!enableGoogleLogin && !enablePassIdLogin && !enableEmailLogin)) && (
            <button
              type="button"
              onClick={handleGoogleSignIn}
              className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-400 hover:to-orange-500 text-slate-950 font-bold text-base shadow-xl shadow-amber-500/20 transition-all transform hover:-translate-y-0.5 flex items-center justify-center space-x-3 group cursor-pointer"
            >
          ...
          {/* DIVIDER (if Google + (Pass ID or Email) are active) */}
          {(enableGoogleLogin || (!enableGoogleLogin && !enablePassIdLogin && !enableEmailLogin)) && (enablePassIdLogin || enableEmailLogin) && (
            <div className="relative my-4">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-[#1c0d1e] px-3 text-amber-300/80 font-mono font-bold tracking-widest text-[10px]">
                  {enablePassIdLogin ? 'OR AUTHENTICATE WITH PASS ID' : 'OR SEARCH WITH REGISTERED EMAIL'}
                </span>
              </div>
            </div>
          )}

          {/* Option B: Pass ID Authentication Form (Only when enablePassIdLogin is true) */}
          {enablePassIdLogin && (
            <form onSubmit={handlePassIdLookup} className="space-y-3.5 text-left bg-[#130915]/80 p-5 rounded-2xl border border-amber-500/20">
              {lookupError && (
          ...
          {/* Option C: Direct Email Lookup (Only when Pass ID is disabled AND Email Login is enabled) */}
          {!enablePassIdLogin && enableEmailLogin && (
        ...
        <div className="space-y-6">
          ...
          {portalViewMode === 'admin' && isAdminUser ? (
            <div className="space-y-4">
              <AdminPanel onClose={() => setPortalViewMode('participant')} />
            </div>
          ) : (
            <div className="space-y-6">
